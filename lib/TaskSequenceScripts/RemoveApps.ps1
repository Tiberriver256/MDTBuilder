...
#            provide a separate AppsToRemove.txt file with a list of apps that
...
function Get-AppList
{
  begin
  {
    ...
    $configFile = "$PSScriptRoot\AppsToRemove.txt"
    ...
    else
    {
      ...
      $logDir = Get-LogDir
      $configFile = "$logDir\AppsToRemove.txt"
      $list | Set-Content $configFile
      Write-Information "Wrote list of apps to $logDir\AppsToRemove.txt, edit and place in the same folder as the script to use that list for future script executions"
    }
